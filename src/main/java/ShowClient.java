...
public class ShowClient extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO dao=new DAO (DataSourceFactory.getDataSource());// on crée un objet de type DAO    
        
        
        response.setContentType("text/html;charset=UTF-8");
        try (PrintWriter out = response.getWriter()) {
            int id = Integer.valueOf(customerID);// On convertit en entier
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ShowClient at " + request.getContextPath() + "</h1>");
            out.println("Le parametre customerID vaut"+   customerID);
            out.println("Le nom du client est:" + customer.getName());
            out.println("</body>");
            out.println("</html>");
        } catch (DAOException ex){
                 Logger.getLogger(ShowClient.class.getName()).log(Level.SEVERE,null
        }
    }
